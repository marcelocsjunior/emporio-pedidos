...
from intelligence.privacy import assert_payload_safe
...
SOURCE_TYPE_REQUEST = "customer_portal.customerorderrequest"

ALERTABLE_REQUEST_STATUSES = {
...
@dataclass(frozen=True, slots=True)
class ActiveRequestNotificationPanel:
...
def _notification_payload(customer_request: CustomerOrderRequest) -> dict:
    item_quantity = customer_request.items.aggregate(total=Sum("quantity"))["total"] or 0
    payload = {
...
def _create_request_notification(
    ...
    customer_request = (
        CustomerOrderRequest.objects.select_related("company")
    ...
    recommendation, created = AIRecommendation.objects.get_or_create(
        ...
        defaults={
            ...
            "summary": "Solicitação recebida e aguardando conferência do Atendimento.",
            "action_suggested": "Abrir a solicitação e realizar a conferência.",
            "evidence": {
                **payload,
...
def notify_request_submitted(customer_request: CustomerOrderRequest) -> bool:
    if not settings.AI_ACTIVE_ASSISTANT_ENABLED:
        return False
    if customer_request.status not in ALERTABLE_REQUEST_STATUSES:
        return False
    try:
        with transaction.atomic():
            _, created = _create_request_notification(customer_request)
        logger.info(
    ...
    *,
    ...
    requests = {
        ...
        for customer_request in CustomerOrderRequest.objects.filter(
            pk__in=request_ids,
            status__in=ALERTABLE_REQUEST_STATUSES,
    ...
    for recommendation in recommendations:
        customer_request = requests.get(recommendation.source_id)
    ...
    return ActiveRequestNotificationPanel(
